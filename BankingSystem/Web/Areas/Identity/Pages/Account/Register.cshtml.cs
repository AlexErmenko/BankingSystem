  ...
  public class RegisterModel : PageModel
  {
    ...
    public async Task<IActionResult> OnPostAsync(string returnUrl = null)
    {
      ...
      if(ModelState.IsValid)
      {
        ...
        IdentityResult result = await _userManager.CreateAsync(user: user, password: Input.Password);
        if(result.Succeeded)
        {
          _logger.LogInformation(message: "User created a new account with password.");

          string code = await _userManager.GenerateEmailConfirmationTokenAsync(user: user);
          code = WebEncoders.Base64UrlEncode(input: Encoding.UTF8.GetBytes(s: code));
          string callbackUrl = Url.Page(pageName: "/Account/ConfirmEmail", pageHandler: null, values: new
          {
            area = "Identity",
            userId = user.Id,
            code
